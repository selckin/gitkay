...
- Any new diff-affecting setting goes in `DiffSettings` only: `DiffCacheKey` *embeds* a `DiffSettings`, so a setting added there is automatically part of the cache key (and read back by the prefetch mapping as `key.settings`) — cached diffs invalidate when it changes with no second edit site to keep in sync
